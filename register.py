...
#     print("Schema creation attempted and search path set to cmti")
#
#
# # Define the entity model - USING THE WORKING VERSION WITHOUT last_login
# class RegisterValues(db.Entity):
...
#
#
# # Try to create the table if it doesn't exist - KEEP ORIGINAL VERSION
# @db_session
...
#         )
#         """)
#         print("Table creation attempted in cmti schema")
...
#
# # Generate mapping - with create_tables=False to avoid conflicts
# db.generate_mapping(create_tables=False)
#
#
# # Setting search path for all sessions
# @app.middleware("http")
# async def set_search_path(request, call_next):
#     @db_session
#     def set_path():
#         db.execute("SET search_path TO cmti, public")
#
...
#
#         employees = select(e for e in RegisterValues)[:]
#
#         employee_list = []
#         for emp in employees:
#             # Since we don't have last_login in the database, we'll default everyone to "Inactive"
#             status = "Inactive"
#
#             # Here you would need to track login activity separately if you need this feature
#
...
#     db.execute("SET search_path TO cmti, public")
#
#     print(f"Current search_path: {db.execute('SHOW search_path').fetchone()[0]}")
#     print(f"Received registration request: {user.dict(exclude={'password'})}")
#
#     # Check if user already exists
#     existing_user = RegisterValues.get(emailid=user.emailid)
#     if existing_user:
#         raise HTTPException(status_code=400, detail="Email already registered")
#
#     try:
#         # Create new user with hashed password
#         new_user = RegisterValues(
...
#             "name": new_user.name,
#             "employee_status": new_user.employee_status
#         }
#     except Exception as e:
...
#     db.execute("SET search_path TO cmti, public")
#
#     print(f"Received login request for email: {user.emailid}")
#
#     # Find user by email
#     db_user = RegisterValues.get(emailid=user.emailid)
#
...
#         )
#
#     print(f"User logged in successfully: {db_user.id}")
#
#     # Return user data (excluding password)
#     return {
#         "id": db_user.id,
#         "emailid": db_user.emailid,
#         "name": db_user.name,
#         "employee_status": db_user.employee_status
#     }
#
#
# # Keep the old endpoint for backward compatibility, but mark it as deprecated
# @app.post("/registerdetails", response_model=UserResponse, deprecated=True)
# @db_session
...
#     uvicorn.run("register:app", host="localhost", port=16000, reload=True)
...
#
#
# # Generate mapping with the base entity
# db.generate_mapping(create_tables=False)
#
#
# # Pydantic models
